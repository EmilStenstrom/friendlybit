---
comments:
- comment_ID: '29976'
  comment_author: Andreas
  comment_author_url: http://exscale.se
  comment_content: I'm just worried that they've put everything in to passing the
    Acid2 test and will still deliver the same old crap.\n\nIE7 was nothing more than
    IE6 with tabs and some patches.\nI'm afraid IE8 will be IE7 with Acid2 compliance
    (but all the other crap still in there...).\n\nI just don't want to get my hopes
    up like I did last time.
  comment_date: '2007-12-20 17:21:13'
  comment_post_ID: '148'
  comment_type: null
  is_admin: '0'
- comment_ID: '29998'
  comment_author: Rasmus Kaj
  comment_author_url: http://www.stacken.kth.se/~kaj/
  comment_content: Maybe because it it doesn't.  There may be a snapshot inside microsoft
    which renders acid 2 perfectly, or they may be lying.  Until they actually deliver
    it, there is no real difference.\n\nOr can I get a copy and check it out for myself?  Today?\n\nUntil
    I can, every version of MSIE sucks big time.  Nothing much to talk about then.
  comment_date: '2007-12-22 21:28:11'
  comment_post_ID: '148'
  comment_type: null
  is_admin: '0'
- comment_ID: '30023'
  comment_author: Blog Surfer &raquo; Visual Studio 2008 Review
  comment_author_url: http://blogsurfer.net/12/visual-studio-2008-review.html
  comment_content: '[...] IE8 renders the acid 2 test I&#8217;m not sure why I don&#8217;t
    see that many people talking about the fact that IE8 renders the acid 2 test perfectly
    now. It really seems Microsoft is starting to move forward again, after starting
    to feel some pressure from the open source world. Will they be able to pick up
    the speed needed to [&#8230;] [...]'
  comment_date: '2007-12-30 09:05:38'
  comment_post_ID: '148'
  comment_type: pingback
  is_admin: '0'
- comment_ID: '30040'
  comment_author: Blog Surfer &raquo; What italic means
  comment_author_url: http://blogsurfer.net/202/what-italic-means-3.html
  comment_content: '[...] IE8 renders the acid 2 test I&#8217;m not sure why I don&#8217;t
    see that many people talking about the fact that IE8 renders the acid 2 test perfectly
    now. It really seems Microsoft is starting to move forward again, after starting
    to feel some pressure from the open source world. Will they be able to pick up
    the speed needed to [&#8230;] [...]'
  comment_date: '2007-12-31 01:01:38'
  comment_post_ID: '148'
  comment_type: pingback
  is_admin: '0'
- comment_ID: '30511'
  comment_author: Emil Stenström
  comment_author_url: http://friendlybit.com
  comment_content: It's live now, and in fact it did render Acid 2 :)
  comment_date: '2008-03-30 16:04:55'
  comment_post_ID: '148'
  comment_type: null
  is_admin: '1'
---